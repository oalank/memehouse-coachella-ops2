...
import {
  Circle, CheckCircle, Clock, AlertCircle, Archive,
  XCircle, Pause, Send, Eye, FileCheck, Ban, Zap
} from "lucide-react";

type StatusType =
  | "active" | "confirmed" | "interviewing" | "offered"
  | "responded" | "screened" | "submitted" | "denied"
  | "high-risk" | "archived" | "completed" | "on-hold"
  | "blocked" | "outreach" | "loa-signed";

const statusConfig: Record<StatusType, { label: string; pillGlow: string; borderColor: string; textColor: string; bgColor: string; icon: React.ElementType }> = {
...
export const StatusPill = ({ status, showIcon = true, className }: StatusPillProps) => {
  ...
  return (
    <span className={cn(
      ...
      config.pillGlow, config.borderColor, config.textColor, config.bgColor,
      className
    )}>
